...
//######################################################################
    ...
    var width          = 1280/1.5;                    // logical canvas width
    var height         = 720/1.5;                     // logical canvas height
    var centrifugal    = 0.24;                     // centrifugal force multiplier when going around curves
    var offRoadDecel   = 0.99;                    // speed multiplier when off road (e.g. you lose 2% speed each update frame)
    var skySpeed       = 0.0011;                   // background sky layer scroll speed when going around curve (or up hill)
    var hillSpeed      = 0.0012;                   // background hill layer scroll speed when going around curve (or up hill)
    var treeSpeed      = 0.0013;                   // background tree layer scroll speed when going around curve (or up hill)
    ...
    var stats          = "";       // mr.doobs FPS counter
...
function getData() {
  ...
  request.open('GET', 'sounds/engine.wav', true);
...
// CAR COLLISION
      for(n = 0 ; n < playerSegment.cars.length ; n++) {
        ...
        if (speed > car.speed) {
          if (Util.overlap(playerX, playerW, car.offset, carW, 0.66)) {
            speed    = car.speed * (car.speed/speed);
            position = Util.increase(car.z, -playerZ, trackLength);
            //hit=true;
            //createPitchStep(-0.083);
            console.log("HIT");
            break;
          }
        }
     ...
     //REV SOUNDS 
	 if(speed <=0){source.playbackRate.value =0;}
	 else{source.playbackRate.value = 5 * Math.round(speed/500)/100;}
    }
//######################################################################	
...
//######################################################################	
    function updateCarOffset(car, carSegment, playerSegment, playerW) {
      ...
      var i, j, dir, segment, otherCar, otherCarW, lookahead = 16, carW = car.sprite.w * SPRITES.SCALE;
      ...
      for(i = 1 ; i < lookahead ; i++) {
        ...
        if ((segment === playerSegment) && (car.speed > speed) && (Util.overlap(playerX, playerW, car.offset, carW, colSens))) {
          if (playerX > 0.5)
            dir = -1;
          else if (playerX < -0.5)
            dir = 1;
        ...
        for(j = 0 ; j < segment.cars.length ; j++) {
          ...
          if ((car.speed > otherCar.speed) && Util.overlap(car.offset, carW, otherCar.offset, otherCarW, 1.1)) {
...
//######################################################################	
    var ROAD = {
      ...
      CURVE:  { NONE: 0, EASY:    2, MEDIUM:    4, HARD:    7 }
    };
//######################################################################	

    function addStraight(num) {
		num = num || 50;
      addRoad(num, num, num/2, 0, 0);
    }

//######################################################################	
    function addCurve(num, curve, height) {
      num    = num    || ROAD.LENGTH.MEDIUM;
      curve  = curve  || -ROAD.CURVE.HARD;
...
//######################################################################	
    function resetRoad() {
      ...
      addStraight();
      addCurve();
      addRoad(0,0,0, -ROAD.LENGTH.SHORT, -lastY()/segmentLength);
      resetSprites();
      resetCars();
...
//######################################################################	    
    function resetCars() {
      ...
      for (var n = 0 ; n < totalCars ; n++) {
        offset = Math.random() * Util.randomChoice([-skillDiff, skillDiff]);
        z      = Math.floor(Math.random() * segments.length) * segmentLength;
        sprite = Util.randomChoice(SPRITES.CARS);
        speed  = maxSpeed/aiHardness + Math.random() * maxSpeed/(sprite == SPRITES.SEMI ? 4 : 2);
...
//######################################################################	
    Game.run({
      ...
      keys: [
        { keys: [KEY.LEFT,  KEY.A, KEY.TLEFT], mode: 'down', action: function() { keyLeft   = true;  } },
